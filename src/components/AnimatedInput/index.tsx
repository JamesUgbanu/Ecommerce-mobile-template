...
const AnimatedTextInput = (props: InputTextProps) => {
  ...
  const animatedIconProps = useAnimatedStyle(() => {
    return {
      ...
      transform: [
        {
          translateX: interpolate(iconSharedValue.value, [0, 1], [8, 0]),
        },
      ],
    };
  });
      ...
      <View style={inputStyles.textInput}>
        ...
        {isError && (
          <Animated.View style={[styles().icon, animatedIconProps]}>
            <AnimatedIcon name={'x'} size={24} color={theme.colors.error} />
          </Animated.View>
        )}
        {!isError && value && (
          <Animated.View style={[styles().icon, animatedIconProps]}>
            <AnimatedIcon name={'check'} size={24} color={theme.colors.success} />
          </Animated.View>
        )}
      </View>
      {isError && (
